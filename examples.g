...
if meshSize == 3 then
    ...
    autGens := Concatenation( autGens, idemGens );

    #### construct 3x3 mesh with grape ####
    D8onMesh := Group( (1,7,9,3)(2,4,8,6), (1,3)(4,6)(7,9) );
    edgeGens := [ [1,2],[2,1], [2,5],[5,2] ];
    mesh := Graph( D8onMesh, [1..9], OnPoints, function(x,y) return [x,y] in edgeGens; end, true );
fi;
...
if meshSize == 4 then
    ...
    idemGens := List( idemGens, x -> PartialPerm( x, x ) );
    autGens := [ phi, r, p ];
    Append( autGens, idemGens );

    #### construct 4x4 mesh with grape ####
    D8onMesh := Group( (1,4,16,13)(2,8,15,9)(3,12,14,5)(6,7,11,10),
      (1,4)(2,3)(5,8)(6,7)(9,12)(10,11)(13,16)(14,15)  );
    edgeGens := [ [1,2],[2,1], [2,3],[3,2], [2,6],[6,2], [6,7],[7,6] ];
    mesh := Graph( D8onMesh, [1..16], OnPoints, function(x,y) return [x,y] in edgeGens; end, true );
fi;
...
return rec( autGens := autGens, mesh := mesh );
end;
